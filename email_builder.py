...
<tr><td style="padding:0 0 6px;">
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border-bottom:2px solid {B};">
    <tr>
      ...
      <td align="right" style="padding:16px 20px 15px;{MONO}font-size:14px;color:{MU};white-space:nowrap;">{short_date}</td>
    </tr>
  </table>
...
def _source_link(text: str, href: str) -> str:
    return (
        f'<a href="{href}" style="{MONO}font-size:14px;color:{B};'
...
def _build_lead(items: list) -> str:
    ...
    for i, item in enumerate(sliced):
        ...
        suffix = f' <span style="{MONO}font-size:14px;font-weight:400;color:{MU};">{src_html}{also_html}</span>' if src_html else ""
...
def _build_news(items: list) -> str:
    ...
    for i, item in enumerate(sliced):
        ...
        also_html = _also_slash(also) if also else ""
        src_html = f' <span style="{MONO}font-size:14px;font-weight:400;color:{MU};">{src}{also_html}</span>' if src else ""
        ...
        rows += (
            ...
            f'<div style="font-size:16px;font-weight:400;line-height:1.35;color:{IN};'
            f'white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">'
...
def _build_diary(items: list) -> str:
    ...
    for i, item in enumerate(sliced):
        ...
        rows += (
            f'<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr>'
            f'<td style="padding:{pad};font-size:16px;line-height:1.35;">'
            f'<div style="font-weight:500;">{event}</div>'
            f'<div style="color:{MU};font-size:15px;">{desc}</div>'
            f'</td>'
            f'<td align="right" valign="top" style="padding:{pad};{MONO}font-size:14px;color:{B};white-space:nowrap;">{dates_html}</td>'
            f'</tr></table>'
        )
...
def _build_numbers(items: list, timestamp: str) -> str:
    ...
    for i, item in enumerate(sliced):
        ...
        rows += (
            ...
            f'<tr><td style="{bg}padding:{pad_top} 20px {pad_bottom};">'
            f'<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr>'
            f'<td style="font-size:16px;font-weight:400;color:{IN};">{name} <span style="color:{MU};{MONO}font-size:14px;font-weight:400;">&middot; {ticker}</span></td>'
            ...
            f'</tr></table>'
            f'<div style="font-size:15px;color:{MU};line-height:1.3;margin-top:2px;">{context}</div>'
            f'</td></tr></table>'
        )
